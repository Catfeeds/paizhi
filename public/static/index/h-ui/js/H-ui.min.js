...
*
* Copyright 2013-2017 鍖椾含棰栨澃鑱斿垱绉戞妧鏈夐檺鍏徃 All rights reserved.
...
! function(a) {
	a.Huiloading = {
		show: function(d) {
			if (a(".loading-wraper").length > 0) {
				a(".loading-wraper").remove()
			}
			if (d == null) {
				d = ""
			}
			var c = '<div class="loading-wraper"><div class="loading-content"><i class="iconpic iconpic-loading"></i> <span>' + d + "</span></div></div>";
			a(document.body).append(c);
			var b = a(".loading-wraper .loading-content").width() + 40;
			a(".loading-wraper .loading-content").css({
				"margin-left": -(b / 2) + "px",
			})
		},
		hide: function() {
...
! function(a) {
	a.fn.Huitab = function(b, d) {
		...
		this.each(function() {
			var e = a(this);
			e.find(b.tabBar).removeClass(b.className);
			e.find(b.tabBar).eq(b.index).addClass(b.className);
			e.find(b.tabCon).hide();
			e.find(b.tabCon).eq(b.index).show();
			e.find(b.tabBar).on(b.tabEvent, function() {
				e.find(b.tabBar).removeClass(b.className);
				a(this).addClass(b.className);
				var f = e.find(b.tabBar).index(this);
				e.find(b.tabCon).hide();
				e.find(b.tabCon).eq(f).show();
				if (d) {
					d()
				}
			})
			...
			d.hover(function() {
				...
				e = setTimeout(function() {
					...
					if (n == "") {
						...
						var j = '<div id="preview-wraper" style="position: absolute;z-index:999;width:' + b.bigImgWidth + "px;height:auto;top:" + r + "px;right:" + h + ";left:" + s + '">' + '<img src="' + f + '" width="' + b.bigImgWidth + '">' + "</div>";
						a("body").append(j);
						var i = new Image();
						i.src = n;
						i.onload = function() {
...
! function(a) {
	a.Huimodalalert = function(c, b) {
		...
		if (b == 0 || typeof(b) == "undefined") {
			a(document.body).append('<div id="modal-alert" class="modal modal-alert radius">' + '<div class="modal-alert-info">' + c + "</div>" + '<div class="modal-footer"> <button class="btn btn-primary radius" onClick="$.Huimodal_alert.hide()">纭畾</button></div>' + "</div>");
...
! function(a) {
	a.Huialert = function() {
		...
		a(document).on("click", ".Huialert i", function() {
...
! function(a) {
	a.Huitotop = function(b) {
		...
		var d = '<a href="javascript:void(0)" class="tools-right toTop Hui-iconfont" title="杩斿洖椤堕儴" alt="杩斿洖椤堕儴" style="display:none;bottom:' + b + 'px">&#xe684;</a>';
