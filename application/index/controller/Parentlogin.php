...
class Parentlogin extends Controller
{
    ...
    public function checkLogin()
    {
        if($this->request->isAjax() && $this->request->isPost()) {
            ...
            $info = DB::name('patriarch')->where('phone','=',$username)->where('status',1)->where('isdelete',0)->find();
            $admin = DB::name('AdminUser')->where('account','=',$username)->where("type",3)->where('status',1)->find();
            if(!$admin)
            {
                $admin = DB::name('AdminUser')->where('mobile','=',$username)->where("type",3)->where('status',1)->find(); 
            }
            if($admin){  
                if($admin['password'] === md5($password)){  
                    ...
                    //将登录id和名称存入session  
						...
						Session::set('phone_account',$info['phone_account']); 
