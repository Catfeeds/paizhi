...
class Tp extends Taglib
{
    ...
    public function tagMenu($tag)
    {
        $menu = isset($tag['menu']) ?
            ...
            ['fadd','add', 'forbid', 'resume', 'delete', 'recyclebin'];
        ...
        foreach ($menu as $k => $m) {
            ...
            switch ($m) {
				case 'echarts':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '图形展示';
                ...
                case 'add':
                    ...
                    break;
				case 'fadd':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '全屏添加';
                    list($url, $param) = $this->parseUrl($url);
                ...
                case 'sedit':
                    ...
                    break;
				case 'fsedit':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '全屏编辑';
                    list($url, $param) = $this->parseUrl($url, 'id=$vo["id"]');
                    $parseStr .= ' <a title="' . $title . '" href="javascript:;" onclick="full_page(\'' . $title . '\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" style="text-decoration:none" class="ml-5"><i class="Hui-iconfont">&#xe6df;</i></a>';
                    break;
				case 'access':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '权限';
                    list($url, $param) = $this->parseUrl($url, 'id=$vo["id"]');
                    $parseStr .= ' <a title="' . $title . '" href="javascript:;" onclick="full_page(\'' . $title . '\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" style="text-decoration:none" class="ml-5"><i class="Hui-iconfont">&#xe6df;</i></a>';
                    break;
                case 'sdelete':
