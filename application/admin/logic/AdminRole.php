...
//            $where_node['id'] = ["in", filter_value($access_node, "node_id")];
              $access_node = Db::name("AdminAccess")->alias("access")->join("__ADMIN_ROLE_USER__ role_user","role_user.role_id = access.role_id")->field("access.node_id")->where("role_user.user_id", UID)->select();
//              $where_node['id'] = ["in", filter_value($access_node, "node_id")];
            $where_node['id'] = filter_value($access_node,"node_id");

            //print_r($where_node['id']);exit;

            $node = array();
            foreach($where_node['id'] as $value){
                $node[] = Db::name("AdminNode")->where("status=1 AND isdelete=0")->where('id',$value)->field("id,pid,group_id,name,title,level,type")->find();
            }


        }else{
            //管理员
            $node = Db::name("AdminNode")->where("status=1 AND isdelete=0")->field("id,pid,group_id,name,title,level,type")->select();//所有的东西
        }
