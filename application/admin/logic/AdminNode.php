...
class AdminNode
{
    ...
    {
        if (ADMIN) {
            $nodes = Db::name("AdminNode")->where("status=1 AND group_id > 0")->field("id,pid,name,group_id,title,type,sort")->order('group_id,pid,sort,id desc')->select();
        } else {
            ...
            $sql = "SELECT node.id,node.name,node.pid,node.group_id,node.title,node.type from "
                ...
                . "AND node.status=1 "
                . "AND node.group_id > 0 "
                . "ORDER BY node.group_id,node.pid,node.sort,node.id desc";
            $nodes = Db::query($sql);
        }
