...
class ArtTutoring extends Controller
{
        ...
        if ($info) {  
          ...
          //  return ajax_return_adv_error($exclePath);
            ...
            foreach ($excel_array as $k => $v) {  
                ...
                $data['phone'] = $v[6]; 
                $data['website'] = $v[7]; 
                $re = Db::name('ArtTutoring')->where('schoolName',$data['schoolName'])->where('schoolNature',$data['schoolNature'])->where('status',1)->where('isdelete',0)->find(); 
                if(!$re){
                    $add = Db::name('ArtTutoring')->insert($data); //批量插入数据  
                }
            } 
            if($add){  
               //$this->success('导入成功');  
