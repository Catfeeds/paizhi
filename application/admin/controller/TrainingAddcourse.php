...
//培训学校---添加课程
class TrainingAddcourse extends Controller
{
...
//            $map['release']=["like", "%" . $info1['realname']. "%"];
    ...
    public function fsedit()
    {
        ...
        if($this->request->isPost()) {
            ...
            $data['target'] = $this->request->param("target"); //课程对象
            $data['courseName'] = $this->request->param("courseName"); //课程名称
            ...
            $data['price'] = $this->request->param("price"); //课程价格
            $data['period'] = $this->request->param("period"); //课时
            $data['startTime'] = $this->request->param("startTime"); //开始时间
            $data['endTime'] = $this->request->param("endTime"); //结束时间
            $data['speaker'] = $this->request->param("speaker"); //主讲人
            $data['type'] = $this->request->param("type"); //课程类型
            $data['feature'] = $this->request->param("feature"); //课程特色
            $data['content'] = htmlspecialchars_decode(html_entity_decode($this->request->param("content")));//课程介绍
            $data['release_time'] = $this->request->param("release_time");
            $data['label'] = $this->request->param("label");


            Db::name("TrainingAddcourse")->where('id',$id)->update($data);
            ...
            if($type == 0){
                ...
                //从名师风采表tp_training_teacher_mien获取所有的老师-------二维数组
                $name = Db::name('TrainingTeacherMien')->field('name,label')->where('status',1)->where('isdelete',0)->select();
            }
            //若当前登录用户是校区
            if($type == 1){
                //获取当前培训学校的名称-----二维数组
                ...
                //从名师风采表tp_training_teacher_mien获取当前校区下的所有的老师---二维数组
                $name = Db::name('TrainingTeacherMien')->field('name,label')->where('schoolAccount',$info['account'])->where('status',1)->where('isdelete',0)->select();
            }
            //若是教师
            if($type == 2){
                ...
                //从名师风采表tp_training_teacher_mien中获取当前教师所在的校区下的名师姓名-----二维数组
                $name = Db::name('TrainingTeacherMien')->field('name,label')->where(array('schoolAccount'=>array('like','%'.$schoolID.'%')))->where('status',1)->where('isdelete',0)->select();
            }
            //若是学生
            if($type == 3){
                ...
                //从名师风采表tp_training_teacher_mien中获取当前学生所在的校区下的所有名师姓名-----二维数组
                $name = Db::name('TrainingTeacherMien')->field('name,label')->where(array('schoolAccount'=>array('like','%'.$schoolID.'%')))->where('status',1)->where('isdelete',0)->select();
            }
            ...
            $this->view->assign('name',$name); //二维数组
            //------------------------------------------------------------------------------------------------------------------------
    ...
    public function fadd()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $path = $this->request->param('path'); //字符串，形如    /uploads/file/20180314/3bf5a01b66d3c.jpg
            if(!$path){
                $data['image'] = '/uploads/file/033fc20efb497505073ed7ff96f84088.png';  //默认课程封面
            }else{
                $data['image'] = $path;
            }
            ...
            Db::name("TrainingAddcourse")->insert($data);
