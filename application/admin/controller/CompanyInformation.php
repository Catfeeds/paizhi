...
//            $map['release']=["like", "%" . $info1['realname']. "%"];
    ...
    public function fsedit()
    {
        ...
        if($this->request->isPost()) {
            ...
            $data['companyName'] = $this->request->param("companyName");//获取选择的公司
            //获取选择的公司名对应的账号
            $schoolInfo = Db::name('SchoolManagement')->where('schoolName',$data['companyName'])->where('status',1)->where('isdelete',0)->find();
            $data['companyAccount'] = $schoolInfo['schoolAccount'];


            $data['registrationNumber'] = $this->request->param("registrationNumber");//注册号
            ...
            $data['content'] = htmlspecialchars_decode(html_entity_decode($this->request->param("content")));//公司简介
            $data['scope'] = $this->request->param('scope');//经营范围

            $data['release_time'] = date('Y-m-d H:i:s',time());//发布时间



            Db::name("CompanyInformation")->where('id',$id)->update($data);
...
//            }
    ...
    public function fadd()
    {
        ...
        if ($this->request->isPost()) {
            ...
            Db::name("CompanyInformation")->insert($data);
