...
//          $map['release']=["like", "%" . $info1['realname']. "%"];
    ...
    public function index()
    {
        ...
        //若是管理员
        if($info['type'] == 0){
            $all_companyName = Db::name('AccountManagement')->where('isdelete',0)->select();
        }
        //若是公司账号登录
        if($info['type'] == 1){
            $all_companyName = Db::name('AccountManagement')->where('schoolAccount',$info['account'])->where('isdelete',0)->select();
        }
        //若是公司员工
        /*if($info['type'] == 2){
