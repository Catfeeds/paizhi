...
//            $map['release']=["like", "%" . $info1['realname']. "%"];
    ...
    public function fsedit()
    {
        ...
        if($this->request->isPost()) {
            ...
            Db::name("Mien")->where('id',$id)->update($data);
    ...
    public function fadd()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if(!$path){
                $data['image'] = '/uploads/file/033fc20efb497505073ed7ff96f84088.png'; //默认封面图片
            }else{
                $data['image'] = $path;
            }
            ...
            $admininfo = Db::name("AdminUser")->field('realname')->where("id", UID)->find();
            $data['release'] = $admininfo['realname']; //获取当前登录用户

            Db::name("Mien")->insert($data);
