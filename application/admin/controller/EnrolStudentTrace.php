...
//展示关于每个报名学生最新的追踪信息
class EnrolStudentTrace extends Controller
{
...
//            $map['release']=["like", "%" . $info1['realname']. "%"];
            $map['schoolAccount']=$info1['account']; //从tp_enrol_student_trace表中查找当前校区下的内容
...
//        $this->datalist($model, $map);
        ...
        //查询关于所有最新的报名信息追踪
        $info = Db::name("AdminUser")->field('realname,account,type')->where("id", UID)->find();
        if($info['type'] == 1)
        {
            //判断是否有搜索条件查询
            if($this->request->param('contacts')!=''){
                $contacts = $this->request->param('contacts');
                $info2 = Db::name('EnrolStudentTrace')->where('schoolAccount',$info['account'])->where('contacts',$contacts)->where('status',1)->where('isdelete',0)->order('id desc')->select();
            }else{
                $info2 = Db::name('EnrolStudentTrace')->where('schoolAccount',$info['account'])->where('status',1)->where('isdelete',0)->order('id desc')->select();
            }
            $all_new_trace_info = array();
            foreach($info2 as $value){
                if(!array_key_exists($value['rid'],$all_new_trace_info))
                {
                    $all_new_trace_info[$value['rid']] = $value; //键表示学生的报名信息id ，值是关于该学生最新的追踪信息---一维数组
                }
        ...
        else if($info['type'] ==2)
        {
            ...
            if($this->request->param('contacts')!=''){
                $contacts = $this->request->param('contacts');
                $info2 = Db::name('EnrolStudentTrace')->where('schoolAccount',$schoolInfo['schoolAccount'])->where('contacts',$contacts)->where('status',1)->where('isdelete',0)->order('id desc')->select();
            }else{
                $info2 = Db::name('EnrolStudentTrace')->where('schoolAccount',$schoolInfo['schoolAccount'])->where('status',1)->where('isdelete',0)->order('id desc')->select();
            }
            $all_new_trace_info = array();
            foreach($info2 as $value){
                if(!array_key_exists($value['rid'],$all_new_trace_info))
        ...
        else if($info['type'] ==0)
        {
            ...
            if($this->request->param('contacts')!=''){
                $contacts = $this->request->param('contacts');
                $info2 = Db::name('EnrolStudentTrace')->where('contacts',$contacts)->where('status',1)->where('isdelete',0)->order('id desc')->select();
            }else{
                $info2 = Db::name('EnrolStudentTrace')->where('status',1)->where('isdelete',0)->order('id desc')->select();
        ...
        $this->view->assign('all_new_trace_info',$all_new_trace_info); //二维数组---存放的是关于学生报名信息的最新的追踪联系
        if($all_new_trace_info){
            $this->view->assign('count',count($all_new_trace_info));
        }else{
...
//            );
//            Db::name('EnrolStudentTrace')->insert($data); //将浏览者的信息也插入到数据表中
//            return $this->view->fetch();
//        }
//
//    }
