...
class EmployeeManagement extends Controller
{
	...
	*/
    ...
    public function add()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            if (
				...
				$data5 = ['schoolName' => $data['schoolName'],'divisionName' => $data['divisionName'], 'className' => $data['className'], 'identity' => $data['identity'],'account' => $data['account']
				,'passWord' => password_hash_tp($data['passWord'])
				,'name' => $data['name'], 'sex' => $data['sex'],'height'=>$data['height'],'weight'=>$data['weight'],'cardID' => $data['cardID'],'postName' => $data['postName'],'iphone' => $data['iphone'],'isUpload'=>'0','isDownloadP'=>'0','isDownloadD'=>'0'
				];
                $ret = $model->isUpdate(false)->save($data5);
                $EId = $model->getLastInsID();
				$user = Db::name("AdminUser");
				$data1 = ['account' => $data['account'], 'realname' => $data['name'], 'password' => password_hash_tp($data['passWord']), 'type' => 2,'mobile' => $data['iphone'],'status' => 1];
				$ret = $user->isUpdate(false)->save($data1);
				$Id = $user->getLastInsID();
				$db_role_user = Db::name("AdminRoleUser");
				$data2 = ['role_id' => 2, 'user_id' => $Id];
				$ret = $db_role_user->isUpdate(false)->save($data2);
				if (array_key_exists("msg",$data)) {
					$data3 = $data['msg'];
					$linkmanDB = Db::name("EmployeeLinkman");
					$linkmanDB->where('employee_id',$EId)->delete();
					$LID = $linkmanDB->max('id')+1;
					foreach($data3 as $key=>$val1){
						$LID = $linkmanDB->max('id')+1;
						$data4 = ['id'=>$LID,'employee_id'=>$EId,'name'=>$val1['name'],'relation'=>$val1['relation'],'number'=>$val1['number'],'isUpload'=>'0','isDownloadP'=>'0','isDownloadD'=>'0'];
			...
			*/
			...
			else if($info['type'] ==2)
			{
				...
				$data_s = Db::name("EmployeeManagement")->field('schoolName')->where('isdelete','0')->where('schoolName',$info1['realname'])->select();
				$division = Db::name("DivisionManagement");
				$data_d = $division->field('divisionName')->where('isdelete','0')->where('schoolName',$info1['realname'])->select();
				$division = Db::name("ClassManagement");
				$data_c = $division->field('class,className')->where('isdelete','0')->where('schoolName',$info1['realname'])->select();
			}
			else if($info['type'] ==3)
			{
				$info1 = Db::name("AdminUser")->field('realname')->where('isdelete','0')->where("id", UID)->find();
				$data_s = Db::name("StudentManagement")->field('schoolName')->where('isdelete','0')->where('schoolName',$info1['realname'])->select();
				$division = Db::name("DivisionManagement");
				$data_d = $division->field('divisionName')->where('isdelete','0')->where('schoolName',$info1['realname'])->select();
				$division = Db::name("ClassManagement");
				$data_c = $division->field('class,className')->where('isdelete','0')->where('schoolName',$info1['realname'])->select();
    ...
    public function edit()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            if (
                ...
                $model = new $modelClass();
				$data2 = ['schoolName' => $data['schoolName'], 'divisionName' => $data['divisionName'], 'className' => $data['className'],'identity' => $data['identity'],'account' => $data['account']
					,'name' => $data['name'], 'sex' => $data['sex'],'height'=>$data['height'],'weight'=>$data['weight'],'cardID' => $data['cardID'],'postName' => $data['postName'],'iphone' => $data['iphone']
					];
                $ret = $model->isUpdate(true)->save($data2, ['id' => $data['id']]);
				$user = Db::name("AdminUser");
				$id = $user->where('account', $data['schoolAccount'])->value('id');
				$data1 = ['account' => $data['account'], 'realname' => $data['schoolName'],'mobile' => $data['iphone']];
				$user->isUpdate(false)->save($data1,['id' => $data['id']]);
				if (array_key_exists("msg",$data)) {
					$data3 = $data['msg'];
					$linkmanDB = Db::name("EmployeeLinkman");
					//$linkmanDB->where('employee_id',$data['id'])->delete();
					//$LID = $linkmanDB->max('id')+1;
					foreach($data3 as $key=>$val1){
					    ...
					    {
					        $LID = $linkmanDB->max('id')+1;
							$data4 = ['id'=>$LID,'employee_id'=>$data['id'],'name'=>$val1['name'],'relation'=>$val1['relation'],'number'=>$val1['number'],'isUpload'=>'0','isDownloadP'=>'0','isDownloadD'=>'0'];
							$linkmanDB->insert($data4);
					    }
					}
				}
                ...
                try {
					...
					$data1 = ['account' => $data['account'], 'realname' => $data['name'],'mobile' => $data['iphone']];
			...
			*/
			...
			$this->view->assign("data_d", $data_d);
			$this->view->assign("data_c", $data_c);
			$linkman = Db::name("EmployeeLinkman")->where("employee_id", $id)->where('isdelete','0')->select();
			$this->view->assign("linkman", $linkman);
			...
			$this->view->assign('shengfen',$shenfen);
    ...
    public function delete()
    {
		...
		{
			for($i = 0; $i<$num; $i++){
				$data = Db::query("select b.id from tp_employee_management a,tp_admin_user b where a.account = b.account and a.id=?",[$arr[$i]]);
    ...
    public function clear()
    {
		...
		$data = Db::query("select b.id from tp_employee_management a,tp_admin_user b where a.account = b.account and a.isdelete = 1");
		foreach($data as $key=>$val){
			if (false === Db::name("AdminRoleUser")->where('user_id',$val['id'])->delete()) {
				return ajax_return_adv_error($model->getError());
			}
          ...
          //  return ajax_return_adv_error($exclePath);
            ...
            foreach ($excel_array as $k => $v) { 
            	if(!empty($v[0])&&!empty($v[1])&&!empty($v[2])){
	                ...
	                $re = Db::name('EmployeeManagement')->where('account',$data['account'])->where('status',1)->where('isdelete',0)->find(); 
	                if($re){
	                	$data['account'] = $v[0].$this->sum(); 
	                }
	                $add = Db::name('EmployeeManagement')->insertgetid($data); //批量插入数据  
	                if($add){
	                    $data1['name'] = $v[6]; 
		                $data1['relation'] = $v[7]; 
		                $data1['number'] = $v[8]; 
		                $data1['employee_id'] = $add; 
		                Db::name('EmployeeLinkman')->insertgetid($data1); //批量插入数据  
	                }
            	}
    ...
    public function dellinkman(){
    	$id = $this->request->param('id');
    	if($id){
    		$re = Db::name('EmployeeLinkman')->where('id',$id)->delete(); 
			$re1 = Db::name('EmployeeLinkmanAccessManagement')->where('EmployeeLinkman_id',$id)->delete(); 
    		if($re&&$re1){
    			exit('1');
    		}else{
                exit('0');
