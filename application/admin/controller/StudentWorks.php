...
//学生作品
class StudentWorks extends Controller
{
...
//            $map['release']=["like", "%" . $info1['realname']. "%"];
    ...
    public function fsedit()
    {
        ...
        if($this->request->isPost()) {
            ...
            $data['works_name'] = $this->request->param("works_name");
            ...
            Db::name("StudentWorks")->where('id',$id)->update($data);
    ...
    public function fadd()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $data['author'] = $this->request->param("author");//作者
            $data['works_name'] = $this->request->param("works_name");//作品名称
            ...
            if(!$path){
                $data['image'] = '/uploads/file/033fc20efb497505073ed7ff96f84088.png';  //默认作品
            }else{
                $data['image'] = $path;
            }
            ...
            $admininfo = Db::name("AdminUser")->field('realname')->where("id", UID)->find();
            $data['release'] = $admininfo['realname']; //当前登录用户的姓名

            Db::name("StudentWorks")->insert($data);
